local ipairs, file, net, table_insert, hook_Add = ipairs, file, net, table.insert, hook.Add
...
function CrateLoadOutItems( ply )
   local itemTable = {}
   for k,v in ipairs( weaponTypes ) do 
      table_insert( itemTable, nil, file.Read( "crates/PLAYER_" .. ply:SteamID64() .. "weaponloadout/" .. v .. ".txt", "DATA" ) )
   end
...
-- Gives the players
hook_Add("Loadout", "PlayersLoadOut", function()
   for k,v in ipairs( weaponTypes ) do
