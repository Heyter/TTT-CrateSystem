...
Adds money to the players total amount
            ...
            Arguments:
               • number amount
---------------------------------------------------------------------------]]
function pmeta:AddMoeny( amount )
...
function pmeta:ItemsForSale()
   ...
   for k,v in pairs(itemsForSale) do
      ...
      idHolder = util.JSONToTable(idHolder)
...
function pmeta:SoldItem( item, price )
   ...
   local data = util.TableToJSON(shopTable, true) -- Its true so if someone wants to go in and change data and it looks pretty :)
